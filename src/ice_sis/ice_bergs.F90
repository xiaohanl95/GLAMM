...
use fms2_io_mod, only: register_axis, register_field, register_restart_field, unlimited
use fms2_io_mod, only: read_restart, write_restart, write_data
...
character(len=32) :: dimnames(4)
  ...
  if (open_file(fileobj, filename, "read", grd%domain, is_restart=.true.)) then
    if (verbose.and.mpp_pe().eq.mpp_root_pe()) write(*,'(2a)') &
      ...
      call register_axis(fileobj, "zaxis_1", size(grd%stored_ice, 3))
      ...
      call register_restart_field(fileobj, 'stored_ice', grd%stored_ice, dimensions=dimnames((/1,2,3,4/)))
      if (variable_exists(fileobj, 'stored_heat')) then
          ...
          call register_restart_field(fileobj, 'stored_heat', grd%stored_heat, dimensions=dimnames((/1,2,4/)))
      else
          if (verbose.and.mpp_pe().eq.mpp_root_pe()) write(*,'(a)') &
...
subroutine sis_write_restart(bergs)
! Arguments
type(icebergs), pointer :: bergs
! Local variables
...
character(len=32) :: dimnames(4)
  ...
  if (open_file(fileobj, filename, "overwrite", bergs%grd%domain, is_restart=.true.)) then
      ...
      !< These are needed so the combiner can work correctly
      call register_field(fileobj, "xaxis_1", "double", (/dimnames(1)/))
      call register_field(fileobj, "yaxis_1", "double", (/dimnames(2)/))

      call register_restart_field(fileobj, 'stored_ice', bergs%grd%stored_ice, dimensions=dimnames((/1,2,3,4/)))
      call register_restart_field(fileobj, 'stored_heat', bergs%grd%stored_heat, dimensions=dimnames((/1,2,4/)))

      call write_restart(fileobj)
      call add_domain_dimension_data(fileobj)
      call close_file(fileobj)
  endif
...
end subroutine sis_write_restart
