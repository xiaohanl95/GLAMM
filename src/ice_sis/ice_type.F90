...
module ice_type_mod
  ...
  use mpp_mod,          only: mpp_pe, mpp_root_pe, mpp_sum, mpp_clock_id, CLOCK_COMPONENT, &
                              CLOCK_LOOP, CLOCK_ROUTINE, stdout,input_nml_file, mpp_broadcast, &
                              mpp_npes
  ...
  use mpp_domains_mod,  only : CENTER, NORTH, EAST
  ...
  use fms2_io_mod,      only: register_restart_field, check_if_open, register_variable_attribute, &
                              register_field, register_axis, read_data, write_data, &
                              read_restart, write_restart, fms2_open_file=>open_file, &
                              get_dimension_size, variable_exists, dimension_exists, file_exists, &
                              FmsNetcdfDomainFile_t, unlimited, fms2_close_file =>close_file, &
                              get_num_dimensions, get_dimension_names, get_global_io_domain_indices, parse_mask_table
  use diag_manager_mod, only: diag_axis_init, register_diag_field, &
                              register_static_field, send_data
  use time_manager_mod, only: time_type, get_time, time_type_to_real
...
public  :: iceClocka,iceClockb,iceClockc
  ...
  type(FmsNetcdfDomainFile_t) :: restart_fileobj ! restart file object opened in ice_model_init
  ...
  subroutine ice_model_init (Ice, Time_Init, Time, Time_step_fast, Time_step_slow, &
    ...
    if (mpp_get_domain_npes(domain) .eq. 1 ) then
      if (.not. associated(mpp_get_io_domain(domain))) &
...
!
    ...
    if (.not.(check_if_open(fileobj))) &
        file_open_success=fms2_open_file(fileobj, trim(restart_file), "read", domain, &
                                         is_restart=.true.)
    if (file_open_success) then
      ! define the global axes and register them to the file object
      ...
      call sis_register_axes(fileobj, dim_names, dim_lengths)
      ! register the restart variables
      ! register the restart fields to the restart file object
      if(variable_exists(fileobj, "part_size")) call register_restart_field(fileobj, "part_size", Ice%part_size)
      if(variable_exists(fileobj, "albedo")) call register_restart_field(fileobj, "albedo", Ice%albedo)
      if(variable_exists(fileobj, "albedo_vis_dir")) call register_restart_field(fileobj, "albedo_vis_dir", Ice%albedo_vis_dir)
      if(variable_exists(fileobj, "albedo_nir_dir")) call register_restart_field(fileobj, 'albedo_nir_dir', Ice%albedo_nir_dir)
      if(variable_exists(fileobj, "albedo_vis_dif")) call register_restart_field(fileobj, 'albedo_vis_dif', Ice%albedo_vis_dif)
      if(variable_exists(fileobj, "albedo_nir_dif")) call register_restart_field(fileobj, 'albedo_nir_dif', Ice%albedo_nir_dif)
      if(variable_exists(fileobj, "rough_mom")) call register_restart_field(fileobj, 'rough_mom', Ice%rough_mom)
      if(variable_exists(fileobj, "rough_heat")) call register_restart_field(fileobj, 'rough_heat', Ice%rough_heat)
      if(variable_exists(fileobj, "rough_moist")) call register_restart_field(fileobj, 'rough_moist', Ice%rough_moist)
      if(variable_exists(fileobj, "t_surf")) call register_restart_field(fileobj, 't_surf', Ice%t_surf)
      if(variable_exists(fileobj, "h_snow")) call register_restart_field(fileobj, 'h_snow', Ice%h_snow(:,:,2:km))
      if(variable_exists(fileobj, "h_ice")) call register_restart_field(fileobj, 'h_ice', Ice%h_ice(:,:,2:km))
      if(variable_exists(fileobj, "t_ice1")) call register_restart_field(fileobj, 't_ice1', Ice%t_ice1(:,:,2:km))
      if(variable_exists(fileobj, "t_ice2")) call register_restart_field(fileobj, 't_ice2', Ice%t_ice2(:,:,2:km))
      if(variable_exists(fileobj, "u_ice")) call register_restart_field(fileobj, 'u_ice', Ice%u_ice)
      if(variable_exists(fileobj, "v_ice")) call register_restart_field(fileobj, 'v_ice', Ice%v_ice)
      if(variable_exists(fileobj, "sig11")) call register_restart_field(fileobj, 'sig11', Ice%sig11)
      if(variable_exists(fileobj, "sig22")) call register_restart_field(fileobj, 'sig22', Ice%sig22)
      if(variable_exists(fileobj, "sig12")) call register_restart_field(fileobj, 'sig12', Ice%sig12)
      if(variable_exists(fileobj, "flux_u")) call register_restart_field(fileobj, 'flux_u', Ice%flux_u)
      if(variable_exists(fileobj, "flux_v")) call register_restart_field(fileobj, 'flux_v', Ice%flux_v)
      if(variable_exists(fileobj, "flux_t")) call register_restart_field(fileobj, 'flux_t', Ice%flux_t)
      if(variable_exists(fileobj, "flux_q")) call register_restart_field(fileobj, 'flux_q', Ice%flux_q)
      if(variable_exists(fileobj, "flux_salt")) call register_restart_field(fileobj, 'flux_salt', Ice%flux_salt)
      if(variable_exists(fileobj, "flux_lw")) call register_restart_field(fileobj, 'flux_lw', Ice%flux_lw)
      if(variable_exists(fileobj, "lprec")) call register_restart_field(fileobj, 'lprec', Ice%lprec)
      if(variable_exists(fileobj, "fprec")) call register_restart_field(fileobj, 'fprec', Ice%fprec)
      if(variable_exists(fileobj, "runoff")) call register_restart_field(fileobj, 'runoff', Ice%runoff)
      if(variable_exists(fileobj, "calving")) call register_restart_field(fileobj, 'calving', Ice%calving)
      if(variable_exists(fileobj, "runoff_hflx")) call register_restart_field(fileobj, 'runoff_hflx', Ice%runoff_hflx)
      if(variable_exists(fileobj, "calving_hflx")) call register_restart_field(fileobj, 'calving_hflx', Ice%calving_hflx)
      if(variable_exists(fileobj, "p_surf")) call register_restart_field(fileobj, 'p_surf', Ice%p_surf)
      if(variable_exists(fileobj, "flux_sw_vis_dir")) call register_restart_field(fileobj, 'flux_sw_vis_dir', Ice%flux_sw_vis_dir)    
      if(variable_exists(fileobj, "flux_sw_vis_dif")) call register_restart_field(fileobj, 'flux_sw_vis_dif', Ice%flux_sw_vis_dif)
      if(variable_exists(fileobj, "flux_sw_nir_dir")) call register_restart_field(fileobj, 'flux_sw_nir_dir', Ice%flux_sw_nir_dir)
      if(variable_exists(fileobj, "flux_sw_nir_dif")) call register_restart_field(fileobj, 'flux_sw_nir_dif', Ice%flux_sw_nir_dif) 
      ! read the data from thte restart file
      call read_restart(fileobj)

      if (.not.(variable_exists(fileobj,"albedo_vis_dir"))) then
      ...
      if (.not.(variable_exists(fileobj,"flux_sw_nir_dir"))) then
        ...
        if(variable_exists(fileobj, "flux_sw")) then
          call read_data(fileobj, "flux_sw", Ice%flux_sw_vis_dir) 
  ...
  subroutine ice_model_end (Ice)
    ...
    ! register and write the restart fields to the restart file
  ...
  ! <DESCRIPTION>
  !> Write out restart files registered to a netCDF file object
  ! </DESCRIPTION>
  subroutine ice_model_restart(Ice, time_stamp)
    type(ice_data_type), intent(in) :: Ice !< structure with all of the data to write to the restart file
    character(len=*), intent(in), optional :: time_stamp !< time stamp to prepend to file name
    ! local
    character(len=10) :: nc_mode ! read, write, overwrite, append
    character(len=200) :: restart_file ! restart file name

    if (present(time_stamp)) then
        restart_file = "RESTART/"//trim(time_stamp)//".ice_model.res.nc"
    else
        restart_file = "RESTART/"//trim(time_stamp)//"ice_model.res.nc"
    endif
    ...
    ! register the data to the restart file
    call register_sis_restart_fields(Ice, trim(restart_file), nc_mode)
    ! write the data to the restart file
    call write_restart(restart_fileobj)

    ! write the dummy dimension data so that the combiner can work correctly
    call add_domain_dimension_data(restart_fileobj)

    if (check_if_open(restart_fileobj)) call fms2_close_file(restart_fileobj)
...
!> register fields and axes to the netCDF restart_file, and write the axes to the file
subroutine register_sis_restart_fields(Ice, file_name, nc_mode)
  type(ice_data_type), intent(in) :: Ice !< structure with the ice fields and metadata
  character(len=*), intent(in) :: file_name !< full path of the restart file
  character(len=*), intent(in) :: nc_mode !< mode of the netcdf file; read, write, overwrite, append

  ! local
  integer :: num_restart_dims ! number of dimensions in the netcdf file
  character(len=32), allocatable :: dim_names(:) ! dimension names
  logical :: file_open_success ! result returned by call to fms2_open_file
  integer :: set1(4), set2(4), set3(3) ! indices for slices of the dim_names and dim_sizes arrays

  ! Open the restart file for writing. Note that this file is closed in ice_model_end
  if (.not.(check_if_open(restart_fileobj))) then
    file_open_success=fms2_open_file(restart_fileobj, trim(file_name), trim(nc_mode), domain, &
      is_restart=.true.)
    if (.not.(file_open_success)) call error_mesg ('register_sis_restart_fields', &
                                       'Unable to open file '//trim(file_name), FATAL)
  endif
  ! get the global axes and register them to the file object
  num_restart_dims = 5
  allocate(dim_names(num_restart_dims))
  dim_names(:) = ""
  ...
  dim_names(5)(1:len_trim("Time")) = "Time"

  set1 = (/1,2,3,5/)
  set2 = (/1,2,4,5/)
  set3 = (/1,2,5/)

  ! register the axes to the file object
  call register_axis(restart_fileobj, "xaxis_1", "x")
  call register_axis(restart_fileobj, "yaxis_1", "y")
  call register_axis(restart_fileobj, "zaxis_1", km)
  call register_axis(restart_fileobj, "zaxis_2", km-1)
  call register_axis(restart_fileobj, "Time", unlimited)

  !< Register the x/y axis as variables so the combiners can work correctly
  call register_field(restart_fileobj, "xaxis_1", "double", (/"xaxis_1"/))
  call register_field(restart_fileobj, "yaxis_1", "double", (/"yaxis_1"/))

  ! register the restart fields to the restart file object
  call register_restart_field(restart_fileobj, "part_size", Ice%part_size, &
                                dimensions=dim_names(set1))
  call register_restart_field(restart_fileobj, "albedo", Ice%albedo, &
                                dimensions=dim_names(set1))
  call register_restart_field(restart_fileobj, "albedo_vis_dir", Ice%albedo_vis_dir, & 
                                dimensions=dim_names(set1))
  call register_restart_field(restart_fileobj, 'albedo_nir_dir', Ice%albedo_nir_dir, & 
                                dimensions=dim_names(set1))
  call register_restart_field(restart_fileobj, 'albedo_vis_dif', Ice%albedo_vis_dif, &
                                dimensions=dim_names(set1))
  call register_restart_field(restart_fileobj, 'albedo_nir_dif', Ice%albedo_nir_dif, &
                                dimensions=dim_names(set1))
  ...
  call register_restart_field(restart_fileobj, 'rough_moist', Ice%rough_moist, &
                                dimensions=dim_names(set1))
  call register_restart_field(restart_fileobj, 't_surf', Ice%t_surf, &
                                dimensions=dim_names(set1))
  call register_restart_field(restart_fileobj, 'h_snow', Ice%h_snow(:,:,2:km), &
                                dimensions=dim_names(set2))
  call register_restart_field(restart_fileobj, 'h_ice', Ice%h_ice(:,:,2:km), &
                                dimensions=dim_names(set2))
  call register_restart_field(restart_fileobj, 't_ice1', Ice%t_ice1(:,:,2:km), &
                                dimensions=dim_names(set2))
  call register_restart_field(restart_fileobj, 't_ice2', Ice%t_ice2(:,:,2:km), &
                                dimensions=dim_names(set2))
  call register_restart_field(restart_fileobj, 'u_ice', Ice%u_ice, &
                                dimensions=dim_names(set3))
  call register_restart_field(restart_fileobj, 'v_ice', Ice%v_ice, &
                                dimensions=dim_names(set3))
  call register_restart_field(restart_fileobj, 'sig11', Ice%sig11, &
                                dimensions=dim_names(set3))
  call register_restart_field(restart_fileobj, 'sig22', Ice%sig22, &
                                dimensions=dim_names(set3))
  call register_restart_field(restart_fileobj, 'sig12', Ice%sig12, &
                                dimensions=dim_names(set3))
  ...
  call register_restart_field(restart_fileobj, 'flux_sw_nir_dif', Ice%flux_sw_nir_dif, &
                                dimensions=dim_names(set3))
end subroutine register_sis_restart_fields

!> register axes to an open netCDF file object
subroutine sis_register_axes(fileobj, axes_names, axes_lengths)
  type(FmsNetcdfDomainFile_t), intent(inout) :: fileobj !< netCDF file object returned by call to fms2_open_file
  character(len=*), dimension(:), intent(in) :: axes_names !< names of the axes
  integer, dimension(:), intent(in) :: axes_lengths !< lengths of the axes
  ! local
  integer :: i
  if (.not.(check_if_open(fileobj))) &
    call error_mesg('sis_register_axes','netCDF file object is not open. Call fms2_open_file(fileobj,...)', FATAL)
  do i=1,size(axes_names,1)
    select case (trim(axes_names(i)))
      case ('xaxis_1'); call register_axis(fileobj,'xaxis_1','x', domain_position=CENTER)
      case ('xaxis_2'); call register_axis(fileobj,'xaxis_2','x', domain_position=CENTER)
      case ('yaxis_1'); call register_axis(fileobj,'yaxis_1','y', domain_position=CENTER)
      case ('yaxis_2'); call register_axis(fileobj,'yaxis_2','y', domain_position=CENTER)
      case default ; call register_axis(fileobj, trim(axes_names(i)), axes_lengths(i))
    end select
  enddo
end subroutine sis_register_axes
