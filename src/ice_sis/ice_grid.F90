...
module ice_grid_mod
  ...
  use mpp_domains_mod, only: SCALAR_PAIR, CGRID_NE, BGRID_NE, mpp_create_super_grid_domain
  use fms_mod,         only: error_mesg, FATAL, stderr
  use fms2_io_mod,     only: open_file, close_file
  use fms2_io_mod,     only: FmsNetcdfFile_t, FmsNetcdfDomainFile_t
  use fms2_io_mod,     only: read_data, write_data, get_global_attribute, get_variable_size
  use fms2_io_mod,     only: get_variable_dimension_names, register_axis, register_field
  use fms2_io_mod,     only: variable_exists, global_att_exists
  use mosaic2_mod,     only: get_mosaic_ntiles, get_mosaic_ncontacts
  use mosaic2_mod,     only: calc_mosaic_grid_area, get_mosaic_contact
...
contains
  ...
  subroutine set_ice_grid(ice_domain, dt_slow, dyn_sub_steps_in, &
    ...
    type(FmsNetcdfFile_t), target       :: grid_fileobj !< Fms2_io fileobj
    type(FmsNetcdfFile_t), pointer      :: ocean_mosaic_fileobj=>null() !< Fms2_io fileobj
    type(FmsNetcdfFile_t), pointer      :: ocean_hgrid_fileobj=>null() !< Fms2_io fileobj
    type(FmsNetcdfDomainFile_t)         :: ocean_topog_fileobj
    type(FmsNetcdfDomainFile_t)         :: grid_fileobj_domain
    type(FmsNetcdfDomainFile_t)         :: ocean_hgrid_fileobj_domain
    integer, dimension(:), allocatable  :: pes !> Current pelist
    logical                             :: coupler_mosaic
    character(len=20)                   :: dim_names(2) !> Array of dimension names


    coupler_mosaic = .false.
    ...
    if ( .not. open_file(grid_fileobj, grid_file, "read", pelist=pes)) then
        call mpp_error(FATAL, 'set_ice_grid: error opening '//trim(grid_file))
    endif

    !--- first determine the grid version
    if(variable_exists(grid_fileobj, 'ocn_mosaic_file') .or. variable_exists(grid_fileobj, 'gridfiles') ) then ! read from mosaic file
       write(outunit,*) '==>Note from ice_grid_mod(set_ice_grid): read grid from mosaic version grid'
       grid_version = VERSION_2
       if( variable_exists(grid_fileobj, 'ocn_mosaic_file') ) then ! coupler mosaic
          coupler_mosaic = .true.
          call read_data(grid_fileobj, "ocn_mosaic_file", ocean_mosaic)
          ocean_mosaic = "INPUT/"//trim(ocean_mosaic)
          allocate(ocean_mosaic_fileobj)
          if ( .not. open_file(ocean_mosaic_fileobj, ocean_mosaic, "read", pelist=pes)) then
              call mpp_error(FATAL, 'set_ice_grid: error opening '//trim(ocean_mosaic))
          endif
       else
          ocean_mosaic = trim(grid_file)
          ocean_mosaic_fileobj => grid_fileobj
       end if
       ...
       call read_data(ocean_mosaic_fileobj, "gridfiles", ocean_hgrid)
       ocean_hgrid = 'INPUT/'//trim(ocean_hgrid)
       allocate(ocean_hgrid_fileobj)
       if ( .not. open_file(ocean_hgrid_fileobj, ocean_hgrid, "read", pelist=pes)) then
            call mpp_error(FATAL, 'set_ice_grid: error opening '//trim(ocean_hgrid))
       endif
    else  if(variable_exists(grid_fileobj, 'x_T')) then
       ocean_hgrid = grid_file
       ocean_hgrid_fileobj => grid_fileobj
       write(outunit,*) '==>Note from ice_grid_mod(set_ice_grid): read grid from new version grid'
       grid_version = VERSION_1
    else if(variable_exists(grid_fileobj, 'geolon_t')) then
       ocean_hgrid = grid_file
       ocean_hgrid_fileobj => grid_fileobj
    ...
    if(grid_version == VERSION_2) then
       if(variable_exists(ocean_mosaic_fileobj, "contacts") ) then
          ncontacts = get_mosaic_ncontacts(ocean_mosaic_fileobj)
          ...
          call get_mosaic_contact( ocean_mosaic_fileobj, tile1(1:ncontacts), tile2(1:ncontacts),           &
       ...
       !--- get grid size
       call get_variable_size(ocean_hgrid_fileobj, 'x', dims(1:2))
       ...
       if(global_att_exists(ocean_hgrid_fileobj, "x_boundary_type")) then
          call get_global_attribute(ocean_hgrid_fileobj, "x_boundary_type", attvalue)
          if(attvalue == 'cyclic') x_cyclic = .true.
       endif

       if( global_att_exists(ocean_hgrid_fileobj, "y_boundary_type") ) then
          call get_global_attribute(ocean_hgrid_fileobj, "y_boundary_type", attvalue)
       ...
       !--- get the grid size
       call get_variable_size(ocean_hgrid_fileobj, 'wet', dims(1:2))
       im = dims(1)
       jm = dims(2)
    end if
    ...
    !< Close the files
    if(grid_version == VERSION_2) then
       if(coupler_mosaic) then
          call close_file(ocean_mosaic_fileobj)
          deallocate(ocean_mosaic_fileobj)

       else !< Here ocean_mosaic is the same as the grid_file
          nullify(ocean_mosaic_fileobj)
       endif

       call close_file(ocean_hgrid_fileobj)
       deallocate(ocean_hgrid_fileobj)
    else !< Here ocean_hgrid is the same as the grid_file
       nullify(ocean_hgrid_fileobj)
    endif
   ...
   if(tripolar_grid) then
    ...
    if(grid_version == VERSION_2) then
       ...
       if ( .not. open_file(ocean_topog_fileobj, ocean_topog, "read", Domain)) then
          call mpp_error(FATAL, 'set_ice_grid: error opening '//trim(ocean_topog))
       endif
       call get_variable_dimension_names(ocean_topog_fileobj, "depth", dim_names)
       call register_axis(ocean_topog_fileobj, dim_names(1), "x")
       call register_axis(ocean_topog_fileobj, dim_names(2), "y")
       call register_field(ocean_topog_fileobj, "depth", "double", dim_names)
       call read_data(ocean_topog_fileobj, 'depth', depth(isc:iec,jsc:jec))
       call close_file(ocean_topog_fileobj)
       do j = jsc, jec
          do i = isc, iec
       ...
       if ( .not. open_file(grid_fileobj_domain, grid_file, "read", Domain)) then
          call mpp_error(FATAL, 'set_ice_grid: error opening '//trim(grid_file))
       endif
       call read_data(grid_fileobj, 'wet', wett(isc:iec,jsc:jec))
    end if
    ...
    case(VERSION_0)
       ...
       call read_data(grid_fileobj_domain, 'geolon_vert_t', geo_lonv)
       call read_data(grid_fileobj_domain, 'geolat_vert_t', geo_latv)
       call read_data(grid_fileobj_domain, 'AREA_OCN', cell_area)
       call close_file(grid_fileobj_domain)
    case(VERSION_1)
       allocate ( x_vert_t(isc:iec,jsc:jec,4), y_vert_t(isc:iec,jsc:jec,4) )
       call read_data(grid_fileobj_domain, 'x_vert_T', x_vert_t)
       call read_data(grid_fileobj_domain, 'y_vert_T', y_vert_t)
       call read_data(grid_fileobj_domain, 'AREA_OCN', cell_area)
       call close_file(grid_fileobj_domain)
    ...
    case(VERSION_2)
       ...
       !< Open the ocean_hgrid file with the supegrid
       if ( .not. open_file(ocean_hgrid_fileobj_domain, ocean_hgrid, "read", Domain2)) then
          call mpp_error(FATAL, 'set_ice_grid: error opening '//trim(ocean_hgrid))
       endif
       call get_variable_dimension_names(ocean_hgrid_fileobj_domain, "x", dim_names)
       call register_axis(ocean_hgrid_fileobj_domain, dim_names(1), "x")
       call register_axis(ocean_hgrid_fileobj_domain, dim_names(2), "y")

       call register_field(ocean_hgrid_fileobj_domain, "x", "double", dim_names)
       call register_field(ocean_hgrid_fileobj_domain, "y", "double", dim_names)

       call read_data(ocean_hgrid_fileobj_domain, 'x', tmpx)
       call read_data(ocean_hgrid_fileobj_domain, 'y', tmpy)
       call close_file(ocean_hgrid_fileobj_domain)
